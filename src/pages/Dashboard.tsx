...
const sidebarItems = [
  ...
  { label: 'Projetos', icon: <FolderIcon />, children: [
      ...
      <Box sx={{ flex: 1, p: 4 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
          <Box>
            ...
            <Typography variant="body1" color="text.secondary">
              Aqui você encontrará um painel dinâmico com informações úteis acerca dos seus testes.
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Avatar alt="Adri Costa" src="https://randomuser.me/api/portraits/men/32.jpg" />
        ...
        <Box sx={{ display: 'flex', gap: 3, mb: 4 }}>
          {metricCards.map((card) => (
            <Paper key={card.label} sx={{ flex: 1, p: 3, bgcolor: card.color, borderRadius: 3, boxShadow: 'none', minWidth: 180 }}>
              ...
              <Typography fontSize={14} color={card.diff < 0 ? 'error.main' : 'success.main'} fontWeight={600}>
                {card.diff < 0 ? '-' : '+'}{Math.abs(card.diff)} essa semana
              </Typography>
        ...
        <Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          <Paper sx={{ flex: 2, p: 3, borderRadius: 3, minWidth: 350, bgcolor: '#FFF6F6' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
              <Typography variant="h6" fontWeight={700}>Saúde do Projeto</Typography>
              <Button variant="text" size="small" sx={{ color: '#888' }} endIcon={<ExpandMoreIcon />}>por período</Button>
            </Box>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={lineData}>
                ...
                <YAxis />
                <Tooltip />
                <Line type="monotone" dataKey="value" stroke="#1890FF" strokeWidth={3} dot={{ r: 5 }} />
              </LineChart>
            </ResponsiveContainer>
          </Paper>
          <Paper sx={{ flex: 1, p: 3, borderRadius: 3, minWidth: 300 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
              <Typography variant="h6" fontWeight={700}>Bugs por categoria</Typography>
              <Button variant="text" size="small" sx={{ color: '#888' }} endIcon={<ExpandMoreIcon />}>por período</Button>
            </Box>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
